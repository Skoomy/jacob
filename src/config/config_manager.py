...
class ConfigManager:
    ...
    def _get_default_config(cls) -> Dict[str, Any]:
        ...
        return {
            "data_sources": asdict(DataSourceConfig(
                google_trends={
                    "geo": "GB",
                    "timeframe": "today 5-y",
                    "category": 0
    ...
    def create_config_template(cls, industry: str, output_path: str) -> None:
        ...
        if industry not in cls.INDUSTRY_TEMPLATES:
            raise ValueError(f"Industry '{industry}' not supported. Choose from: {list(cls.INDUSTRY_TEMPLATES.keys())}")
        ...
        logger.info(f"Configuration template created for {industry} industry at {output_path}")
