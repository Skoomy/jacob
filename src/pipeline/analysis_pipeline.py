...
class AnalysisPipeline:
    ...
    def _collect_data(self):
        ...
        logger.info(f"Data collection completed. Total datasets: {len(self.collected_data)}")
    ...
    def _create_visualizations(self):
        ...
        if "google_trends" in self.processed_data:
            visualizations["trends_timeseries"] = self.visualizer.create_trends_timeseries(
                self.processed_data["google_trends"]
            )
        
        # Market share plots
        if "market_data" in self.processed_data:
            visualizations["market_share_plot"] = self.visualizer.create_market_share_plot(
                self.processed_data["market_data"]
            )
        
        # Model results plots
        if hasattr(self, 'model_results'):
            visualizations["model_results"] = self.visualizer.create_model_results_plots(
                self.model_results
            )
        
        # Correlation analysis
        visualizations["correlation_analysis"] = self.visualizer.create_correlation_plot(
            self.processed_data
        )
    ...
    def _prepare_model_data(self) -> Dict[str, Any]:
        ...
        if "google_trends" in self.processed_data:
            ...
            model_data["share_of_voice"] = trends_data.div(trends_data.sum(axis=1), axis=0).fillna(0)
