...
class DataProcessor:
    ...
    def process_all_data(self, data: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        if "google_trends" in data:
            processed_data["google_trends"] = self._process_google_trends(data["google_trends"])
        ...
        if "uk_vehicle" in data:
            processed_data["uk_vehicle"] = self._process_uk_vehicle_data(data["uk_vehicle"])
    ...
    def _process_google_trends(self, trends_data: pd.DataFrame) -> pd.DataFrame:
        ...
        self.processing_summary.append({
            ...
            "processing_steps": ["datetime_index", "numeric_only", "fill_na", "normalize"]
        })
    ...
    def _process_uk_vehicle_data(self, vehicle_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            if any(keyword in col.lower() for keyword in ["electric", "hybrid", "petrol", "diesel", "registration"]):
        ...
        self.processing_summary.append({
            ...
            "processed_shape": df.shape,
            "processing_steps": ["clean_columns", "select_relevant", "convert_numeric", "fill_na"]
        })
        
        logger.info(f"Processed UK vehicle data: {df.shape}")
        return df
    
    def align_time_series(self, data: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        for dataset_name, df in data.items():
            if isinstance(df, pd.DataFrame):
                ...
                if not numeric_df.empty:
                    ...
                    numeric_df.columns = [f"{dataset_name}_{col}" for col in numeric_df.columns]
